...
class BasicController extends Controller
{
  ...
  public function reorder(Request $request, $id)
  {
    try {
      ...
      $allModels = $query->orderBy('order_index', 'asc')->get();
      ...
      return response()->json([
        'success' => false,
        'message' => 'Error al reordenar registros: ' . $th->getMessage() . ' Ln.' . $th->getLine()
      ], 500);
    }
  }
  ...
  public function reactView(Request $request)
  {
    ...
    $properties = [
      'session' => Auth::user(),
      'messagesServiceCount' => Message::where('status', true)->where('seen', false)->whereNotNull('service_id')->count(),
      'messagesGeneralCount' => Message::where('status', true)->where('seen', false)->whereNull('service_id')->count(),
      ...
      'global' => [
        ...
        'API_KEY_TINYMCE' => General::where('correlative', 'API_KEY_TINYMCE')->value('description') ?? env('TINYMCE_API_KEY')
      ],
  ...
  public function paginate(Request $request): HttpResponse|ResponseFactory|\Illuminate\Http\JsonResponse
  {
    ...
    try {
      ...
      // Aplicar with dinámicamente e instancia básica
      $instance = $this->setPaginationInstance($request, $this->model)
        ->when($langId && Schema::hasColumn((new $this->model)->getTable(), 'lang_id'), function ($query) use ($langId) {
          $query->where($this->prefix4filter ? $this->prefix4filter . '.lang_id' : 'lang_id', $langId);
        })
        ->with($withRelations);
  ...
  public function save(Request $request): HttpResponse|ResponseFactory
  {
    ...
    try {
      ...
      // Asignar lang_id si el modelo lo tiene y es un nuevo registro
      $langId = app('current_lang_id');
      $table = (new $this->model)->getTable();
      $isNew = !isset($body['id']) || empty($body['id']) || $body['id'] === 'undefined' || $body['id'] === 'null';
      if (Schema::hasColumn($table, 'lang_id') && !isset($body['lang_id']) && $isNew) {
        $body['lang_id'] = $langId;
      }
